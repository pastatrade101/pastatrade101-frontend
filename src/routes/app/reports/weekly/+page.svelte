<script lang="ts">
  ...
  onMount(async () => {
    try {
      bundle = await api('/reports/weekly/latest', { auth: true });
    } catch (e) {
      error = e instanceof Error ? e.message : 'No weekly report available yet.';
    } finally {
      loading = false;
    }
  });
...
{#if loading}
  <p class="text-sm text-muted">Loading latest weekly report…</p>
{:else if error}
