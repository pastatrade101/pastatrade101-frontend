<script lang="ts">
  ...
  onMount(async () => {
    try {
      bundle = await api('/reports/daily/latest', { auth: true });
    } catch (e) {
      error = e instanceof Error ? e.message : 'No daily report available yet.';
    } finally {
      loading = false;
    }
  });
...
{#if loading}
  <p class="text-sm text-muted">Loading latest daily report…</p>
{:else if error}
